 ...
 * $Id$
 */
package it.csi.mddtools.guigen.genutils.responsive;
 ...
 */
public class GenUtilsLayoutResponsive {
	...
	public static String getMenuPanelOrientation(MenuPanel mp) {
		...
		// il MenuPanel pu� essere contenuto solo in un FormPanel (da check)
		FormPanel parent = (FormPanel)mp.eContainer();
		
		// a che livello di annidamento � inserito il MenuPanel?
		int level = getMenuPanelLevel(parent);
		
		
		if ( level == 1 ) {
			// il menu � contenuto in un FormPanel di primo livello
			if ( parent.getLayout() instanceof VerticalFlowPanelLayout ) {
				res = " horizontal";
			}
		} 
		else if ( level == 2 ) {
			// il menu � contenuto in un FormPanel di secondo livello...
			if ( parent.getLayoutSpec() != null && parent.getLayoutSpec() instanceof UDLRCWidgetLayoutSpec ) {
				// con layout UDLRC: in quale quadrante �?
			...
			// e se � annidato pi� profondamente?
		}
	...
	public static String getButtonDivStyleByLayout(GUIModel model, Button b) {
		...
		if ( ((UDLRCWidgetLayoutSpec)b.getLayoutSpec()).getValue() ==  UDLRCSpecConstants.LEFT ) {
			...
			// per introdurre il caso CENTER � stato necessario modificare i CHECK (prima non era permesso)
	...
	public static String getCustomtagHeaderColspan(WidgetsPanel wp, Widget w) {
		...
		if ( w instanceof Table || w instanceof Button || w instanceof ConfirmButton || w instanceof ResetButton ) {
			...
			// Per gli altri widgets, se l'attributo "label" � null non mette la colonna della label
			colspan = colspan + 1;
		}
